...
| R# | 規則 | Why |
|----|------|-----|
| R1 | 只在 **macOS host** 跑（不用 docker / Xvfb） | docker 不是真實使用環境；macOS 的 Qt build / Metal / focus / IME 才是 |
| R2 | 鍵盤 **必須** 透過 `osascript`（外部 OS event）。**禁止** 用 `limn:call`、`(funcall ...)`、wire JSON 取代按鍵 | 把 binary 當黑盒 |
| R3 | 驗證 **必須** 走 `screencapture` + script。**禁止** 用 `view/get`、`buffer-list`、`*messages*` 取代真實 UI 觀察 | wire state ≠ user-visible state |
