...
(let* ((sock (format nil "/tmp/limn-w17-~a" (sb-posix:getpid))) (limn-bin "/limn/sioyek/limn")
  ...
  ;; v0.39 W17: open A first, kill its line, THEN open B and yank.
  ;; find-file engine-loads text → switches the visible widget to that
  ;; buffer.  Doing both finds up-front (old order) left text-mode
  ;; pointing at B (empty) so C-SPC/C-e/C-w operated on empty buffer.
  (let ((bid-a (limn/file:find-file *path-a*)))
    ...
    (let ((bid-b (limn/file:find-file *path-b*)))
