  ...
  (let ((pkg (gensym "PKG")))
    `(let* ((,var (make-mock-buf24 :id ,id :text ,text :cursor ,cursor))
       ...
       (if (null ,pkg)
           ...
           (progn ,@body)
           (let* ((insert-sym   (find-symbol "*BUFFER-INSERT-FN*"     ,pkg))
                  (delete-sym   (find-symbol "*BUFFER-DELETE-FN*"     ,pkg))
                  (cursor-sym   (find-symbol "*BUFFER-CURSOR-FN*"     ,pkg))
                  (setcur-sym   (find-symbol "*BUFFER-SET-CURSOR-FN*" ,pkg)))
             (progv
                 (remove nil (list insert-sym delete-sym cursor-sym setcur-sym))
                 (list
                  (lambda (bid off str)
                    (declare (ignore bid)) (mbuf-insert-at! ,var off str))
                  (lambda (bid from len)
                    (declare (ignore bid)) (mbuf-delete! ,var from len))
                  (lambda (bid)
                    (declare (ignore bid)) (mbuf-cursor ,var))
                  (lambda (bid off)
                    (declare (ignore bid)) (setf (mbuf-cursor ,var) off)))
               ,@body))))))
...
(defmacro with-mark-buf ((var &key (id "b") (text "") (cursor 0)) &body body)
  ...
  (let ((pkg (gensym "PKG")))
    `(let* ((,var (make-mock-buf24 :id ,id :text ,text :cursor ,cursor))
       ...
       (if (null ,pkg)
           (progn ,@body)
           (let* ((cursor-sym   (find-symbol "*BUFFER-CURSOR-FN*"     ,pkg))
                  (setcur-sym   (find-symbol "*BUFFER-SET-CURSOR-FN*" ,pkg)))
             (progv
                 (remove nil (list cursor-sym setcur-sym))
                 (list
                  (lambda (bid)
                    (declare (ignore bid)) (mbuf-cursor ,var))
                  (lambda (bid off)
