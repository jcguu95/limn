...
(defsystem #:limn
  ...
  (
    ...
    (:file "limn-orderless" :depends-on ("limn-completion"))
    (:file "limn-vertico" :depends-on ("limn-orderless"))
    ;; minad P4: Corfu (in-buffer popup) + Cape (capf 後端) + Embark (情境動作)
    (:file "limn-corfu" :depends-on ("limn-vertico"))
    (:file "limn-cape" :depends-on ("limn-corfu"))
    (:file "limn-embark" :depends-on ("limn-cape"))
    (:file "limn-consult" :depends-on ("limn-embark"))
    (:file "limn-marginalia" :depends-on ("limn-consult"))
    (:file "limn-isearch" :depends-on ("limn-marginalia"))
    (:file "limn-occur" :depends-on ("limn-isearch"))
    (:file "limn-ibuffer" :depends-on ("limn-occur"))
    (:file "limn-pdf-mode" :depends-on ("limn-ibuffer"))
